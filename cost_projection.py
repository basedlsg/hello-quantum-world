...
def calculate_experiment_cost(max_qubits: int, trials_per_size: int = 3, 
                            noise_levels: int = 3, simulator: str = 'DM1') -> dict:
    ...
    for n_qubits in range(2, max_qubits + 1):
        ...
        # Time per trial
        time_per_trial = estimate_simulation_time(n_qubits, simulator) * circuits_per_trial
                ...
                for noise_levels in [1, 2, 3]:
                    ...
                    if cost_analysis['total_cost_usd'] <= budget_usd:
                        strategies.append({
                            ...
                            'scientific_value': max_qubits * trials * noise_levels  # Heuristic
                        })
...
def main():
    ...
    print(f"Pricing: SV1=${PRICING['SV1']}/min, DM1=${PRICING['DM1']}/min, TN1=${PRICING['TN1']}/min")
    
    # Test current experiment scaling
    print("\n--- Current Experiment Scaling (DM1) ---")
    for max_qubits in [6, 8, 10, 12]:
        cost_analysis = calculate_experiment_cost(max_qubits, trials_per_size=3, 
                                                noise_levels=2, simulator='DM1')
        print(f"{max_qubits} qubits: {cost_analysis['total_minutes']:.1f} min, "
    ...
    for i, strategy in enumerate(strategies[:5]):
        print(f"{i+1}. {strategy['max_qubits']} qubits, {strategy['simulator']}, "
              f"{strategy['trials']} trials, {strategy['noise_levels']} noise levels")
        print(f"   Cost: ${strategy['total_cost']:.2f}, Time: {strategy['total_minutes']:.1f} min")
    ...
    if strategies:
        ...
        print(f"Free tier usage: {min(detailed_cost['total_minutes'], 60):.1f}/60 minutes")
