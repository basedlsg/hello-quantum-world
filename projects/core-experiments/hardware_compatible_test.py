...
def run_hardware_compatible_experiment(shots: int = 1000):
    ...
    for n_qubits in range(2, 8):  # Up to 7 qubits per cost analysis
        ...
        ideal_spatial_probs = calculate_probability_distributions(
            ideal_spatial_counts, shots
        )
        ideal_nonspatial_probs = calculate_probability_distributions(
            ideal_nonspatial_counts, shots
        )
        ...
        if spatial_sum > 0:
            noisy_spatial_probs = {
                k: v / spatial_sum for k, v in noisy_spatial_probs.items()
            }
        if nonspatial_sum > 0:
            noisy_nonspatial_probs = {
                k: v / nonspatial_sum for k, v in noisy_nonspatial_probs.items()
            }

        # Calculate Hellinger distances (noise impact)
        spatial_hellinger = hellinger_distance(
            ideal_spatial_probs, noisy_spatial_probs, n_qubits
        )
        nonspatial_hellinger = hellinger_distance(
            ideal_nonspatial_probs, noisy_nonspatial_probs, n_qubits
        )
        ...
        print(
            f"Hellinger distances - Spatial: {spatial_hellinger:.6f}, Non-spatial: {nonspatial_hellinger:.6f}"
        )
        print(
            f"Fidelity-like metrics - Spatial: {spatial_fidelity:.6f}, Non-spatial: {nonspatial_fidelity:.6f}"
        )
