...
def main():
    ...
    for n_qubits in range(2, 7):  # 2 to 6 qubits
        ...
        nonspatial_props = analyze_circuit_properties(
            nonspatial_circuit, "nonspatial", n_qubits
        )
        ...
        # Print comparison
        print(
            f"Spatial:     Total gates={spatial_props['total_gates']:2d}, "
            f"CNOTs={spatial_props['cnot_count']:2d}, Depth={spatial_props['depth']:2d}"
        )
        print(
            f"Non-spatial: Total gates={nonspatial_props['total_gates']:2d}, "
        ...
        cnot_ratio = (
            nonspatial_props["cnot_count"] / spatial_props["cnot_count"]
            if spatial_props["cnot_count"] > 0
            else float("inf")
        )
        depth_ratio = nonspatial_props["depth"] / spatial_props["depth"]

        print(
            f"Ratios (non-spatial/spatial): Gates={gate_ratio:.2f}, CNOTs={cnot_ratio:.2f}, Depth={depth_ratio:.2f}"
        )
        print()
    ...
    for i, n_qubits in enumerate(range(2, 7)):
        ...
        print(
            f"  {n_qubits} qubits: {s_gates} vs {ns_gates} gates (ratio: {ratio:.2f})"
        )
    ...
    for i, n_qubits in enumerate(range(2, 7)):
        ...
        print(
            f"  {n_qubits} qubits: {s_depth} vs {ns_depth} depth (ratio: {ratio:.2f})"
        )
    ...
    print(
        "Look for correlation between these ratios and our observed fidelity crossover at 5-6 qubits."
    )
