...
class RealisticQuantumResearch:
    ...
    def week1_entanglement_studies(self):
        ...
        with Tracker() as tracker:
            cloud_result = (
                self.devices["sv1_simulator"].run(bell_circuit, shots=1000).result()
            )
            sim_cost = (
                float(tracker.simulator_tasks_cost())
                if tracker.simulator_tasks_cost()
                else 0.0
            )
        ...
        try:
            with Tracker() as tracker:
                ionq_task = self.devices["ionq_aria"].run(
                    bell_circuit, shots=10
                )  # Small test
        ...
        try:
            with Tracker() as tracker:
                rigetti_task = self.devices["rigetti_ankaa"].run(
                    bell_circuit, shots=50
                )  # Larger test
    ...
    def week3_comparative_analysis(self):
        ...
        for platform in platforms:
            ...
            if "qubits" in platform:
                print(
                    f"      Qubits: {platform['qubits']}, Cost/shot: ${platform['shot_cost']:.5f}"
                )
            else:
                print(
                    f"      Atoms: {platform['atoms']}, Cost/shot: ${platform['shot_cost']:.5f}"
                )
    ...
    def generate_research_summary(self):
        ...
        print(
            f"   ✅ Project completed {((self.total_budget - self.spent)/self.total_budget)*100:.1f}% under budget!"
        )
