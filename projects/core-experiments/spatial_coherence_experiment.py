...
class SpatialCoherenceExperiment:
    ...
    def run_coherence_comparison(self, n_qubits: int, noise_levels: List[float]) -> dict:
        """
        ...
        for noise_prob in noise_levels:
            ...
            nonspatial_noisy = self.add_decoherence(nonspatial_circuit.copy(), noise_prob)
            nonspatial_fidelity = self.measure_fidelity(nonspatial_circuit, nonspatial_noisy)
    ...
    def run_scaling_study(self, qubit_counts: List[int], noise_level: float) -> dict:
        ...
        for n_qubits in qubit_counts:
            ...
            nonspatial_circuit = self.create_nonspatial_circuit(n_qubits)
            nonspatial_noisy = self.add_decoherence(nonspatial_circuit.copy(), noise_level)
            nonspatial_fidelity = self.measure_fidelity(nonspatial_circuit, nonspatial_noisy)
...
def main():
    ...
    basic_results = experiment.run_coherence_comparison(n_qubits=4, noise_levels=noise_levels)
    ...
    for i, noise in enumerate(noise_levels):
        ...
        print(f"Noise {noise:.3f}: Spatial={spatial_fid:.3f}, "
    ...
    for i, n_qubits in enumerate(qubit_counts):
        if i < len(scaling_results['spatial_scaling']):
            ...
            print(f"{n_qubits} qubits: Spatial={spatial_fid:.3f}, "
