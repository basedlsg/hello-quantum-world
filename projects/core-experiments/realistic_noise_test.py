...
def run_realistic_noise_experiment():
    ...
    print(
        f"T1 = {T1*1e6:.1f} μs, T2 = {T2*1e6:.1f} μs, Gate time = {GATE_TIME*1e9:.0f} ns"
    )
    ...
    for n_qubits in range(2, 7):
        ...
        ideal_nonspatial_result = (
            device.run(ideal_nonspatial, shots=0).result().values[0]
        )
        noisy_spatial_result = device.run(noisy_spatial, shots=0).result().values[0]
        noisy_nonspatial_result = (
            device.run(noisy_nonspatial, shots=0).result().values[0]
        )
    ...
    advantage_results = []
    for n_qubits in range(2, 7):
        spatial_fid = df[(df["n_qubits"] == n_qubits) & (df["type"] == "spatial")][
            "fidelity"
        ].iloc[0]
        nonspatial_fid = df[
            (df["n_qubits"] == n_qubits) & (df["type"] == "nonspatial")
        ]["fidelity"].iloc[0]
