...
class AdvancedCoherenceExperiment:
    ...
    def __init__(self, device_name: str):
        if device_name.startswith("arn:"):
            ...
            raise ValueError(
                "Invalid device specified. Use 'local_dm', 'sv1', or a valid AWS device ARN."
            )
        self.results = {}
    ...
    def create_nonspatial_circuit(
        self, n_qubits: int, noise_prob: float = 0.0
    ) -> Circuit:
            ...
            for j in range(i + 2, n_qubits):  # Skip nearest neighbors
                ...
                if noise_prob > 0:
                    circuit.apply_gate_noise(
                        noises.Depolarizing(probability=noise_prob)
                    )
    ...
    def run_scaling_study(
        self, max_qubits: int, noise_prob: float, trials: int
    ) -> dict:
        """Study how coherence preservation scales with system size, with statistical rigor."""
        print(
            f"Running scaling study: max_qubits={max_qubits}, noise_prob={noise_prob}, trials={trials}"
        )
        ...
        for n_qubits in range(2, max_qubits + 1):
            ...
            for i in range(trials):
                ...
                nonspatial_fidelity = self.measure_fidelity(
                    ideal_nonspatial, noisy_nonspatial
                )
                nonspatial_fidelities_trial.append(nonspatial_fidelity)

                print(
                    f"  Trial {i+1}/{trials}: Spatial Fid={spatial_fidelity:.3f}, Non-Spatial Fid={nonspatial_fidelity:.3f}"
                )

            # Calculate mean and standard error of the mean (SEM)
            all_results["spatial_means"].append(np.mean(spatial_fidelities_trial))
            all_results["spatial_sems"].append(
                np.std(spatial_fidelities_trial) / np.sqrt(trials)
            )
            all_results["nonspatial_means"].append(np.mean(nonspatial_fidelities_trial))
            all_results["nonspatial_sems"].append(
                np.std(nonspatial_fidelities_trial) / np.sqrt(trials)
            )
    ...
    with open(filepath, "w", newline="") as f:
        ...
        writer.writerow(
            [
                ...
                "spatial_fidelity_sem",
                "nonspatial_fidelity_mean",
                "nonspatial_fidelity_sem",
            ]
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run an advanced coherence experiment."
    )
    parser.add_argument(
        "--device",
        type=str,
        default="local_dm",
        help="Device to run on ('local_dm', 'sv1', or AWS ARN).",
    )
    parser.add_argument(
        "--max-qubits",
        type=int,
        default=6,
        help="Maximum number of qubits for the scaling study.",
    )
    parser.add_argument(
        "--noise-p",
        type=float,
        default=0.005,
        help="Probability of depolarizing noise.",
    )
    parser.add_argument(
        "--trials",
        type=int,
    ...
    results = experiment.run_scaling_study(
        max_qubits=args.max_qubits, noise_prob=args.noise_p, trials=args.trials
    )
