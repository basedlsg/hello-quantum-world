...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'aws_study_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
...
class AWSConstrainedQuantumStudy:
    ...
    def execute_with_budget_check(self, device_name: str, circuit: Circuit, shots: int) -> Dict[str, Any]:
        ...
        if self.total_spent + estimated_cost > self.budget_limit:
            logger.warning(f"Budget exceeded! Estimated cost: ${estimated_cost:.2f}, Remaining: ${self.budget_limit - self.total_spent:.2f}")
            return {'status': 'budget_exceeded', 'estimated_cost': estimated_cost}
        
        logger.info(f"Executing on {device_name} - Estimated cost: ${estimated_cost:.2f}")
        ...
        try:
            ...
            with Tracker() as tracker:
                ...
                if device_name == 'local_simulator':
                    ...
                    result = task.result()
                    actual_cost = float(tracker.qpu_tasks_cost()) if tracker.qpu_tasks_cost() else estimated_cost
                ...
                logger.info(f"Task completed - Actual cost: ${actual_cost:.2f}, Remaining budget: ${remaining_budget:.2f}")
    ...
    def week1_entanglement_characterization(self) -> Dict[str, Any]:
        ...
        for device_name, shots in test_sequence:
            ...
            if result['status'] == 'success':
                ...
                logger.info(f"{device_name}: Bell fidelity = {bell_fidelity:.3f}, Cost = ${result['cost']:.2f}")
        ...
        for circuit_name, circuit in spatial_circuits:
            ...
            if week2_spent + 1.0 < week2_budget:  # Estimate $1 for cloud sim
                ...
                if result['status'] == 'success':
                    ...
                    logger.info(f"Completed {circuit_name} on SV1 - Cost: ${result['cost']:.2f}")
    ...
    def week3_algorithm_comparison(self) -> Dict[str, Any]:
        ...
        for gamma, beta in parameter_sets:
            ...
            result = self.execute_with_budget_check('local_simulator', maxcut_circuit, 1000)
            if result['status'] == 'success':
                probs = result['result'].measurement_probabilities
                ...
                expected_cut = sum(cut_values[state] * prob for state, prob in probs.items())
                ...
                logger.info(f"QAOA γ={gamma}, β={beta}: Expected cut = {expected_cut:.3f}")
            ...
            if week3_spent + 5.0 < week3_budget:  # Conservative estimate
                if self.check_device_availability('rigetti_ankaa'):
                    result = self.execute_with_budget_check('rigetti_ankaa', maxcut_circuit, 100)
                    if result['status'] == 'success':
                        probs = result['result'].measurement_probabilities
                        expected_cut = sum(cut_values[state] * prob for state, prob in probs.items())
                        ...
                        logger.info(f"QAOA on Rigetti γ={gamma}, β={beta}: Expected cut = {expected_cut:.3f}, Cost = ${result['cost']:.2f}")
    ...
    def week4_scaling_analysis(self) -> Dict[str, Any]:
        ...
        if remaining_budget > 2.0:
            ...
            if result['status'] == 'success':
                ...
                logger.info(f"12 qubits on SV1: {result['execution_time']:.3f}s, Cost: ${result['cost']:.2f}")
    ...
    def generate_final_report(self) -> Dict[str, Any]:
        ...
        # Save report
        report_filename = f'aws_quantum_study_report_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
        with open(report_filename, 'w') as f:
            json.dump(report, f, indent=2)
        
        logger.info(f"Final report saved to: {report_filename}")
        logger.info(f"Total cost: ${self.total_spent:.2f} (Budget: ${self.budget_limit})")
    ...
    def _analyze_results(self) -> Dict[str, str]:
        ...
        if 'week1_entanglement' in self.experimental_data:
            ...
            if 'statistics' in week1_data:
                ...
                findings['entanglement_fidelity'] = f"Mean Bell state fidelity: {stats['mean_fidelity']:.3f} ± {stats['std_fidelity']:.3f}"
        
        # Week 2 analysis
        if 'week2_spatial' in self.experimental_data:
            week2_data = self.experimental_data['week2_spatial']
            findings['spatial_simulation'] = f"Tested {week2_data['circuits_tested']} spatial circuit configurations"
        
        # Week 3 analysis
        if 'week3_algorithms' in self.experimental_data:
            week3_data = self.experimental_data['week3_algorithms']
            findings['algorithm_performance'] = f"QAOA MaxCut tested with {len(week3_data['parameter_sets'])} parameter sets"
        
        # Week 4 analysis
        if 'week4_scaling' in self.experimental_data:
            week4_data = self.experimental_data['week4_scaling']
            findings['scaling_limits'] = f"Tested up to {week4_data['max_qubits_tested']} qubits within budget constraints"
    ...
    def _analyze_costs(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'total_spent': self.total_spent,
            'most_expensive_week': max(weekly_costs.items(), key=lambda x: x[1]) if weekly_costs else None,
            'cost_per_measurement': self.total_spent / sum(len(data.get('measurements', [])) for data in self.experimental_data.values())
        }
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if self.total_spent < self.budget_limit * 0.8:
            recommendations.append("Study completed under budget - consider expanding scope in future iterations")
