"""
Working AWS FMO Demonstration - handles both local (numpy) and AWS (list) formats
"""
...
def extract_single_excitation_populations(density_matrix):
    """Extract populations from the single-excitation subspace - handles both numpy and list formats."""
    
    # Handle both local (numpy) and AWS (list) formats
    ...
    for state_idx in single_exc_states:
        # Extract diagonal element and convert to real Python float
        pop_complex = dm[state_idx, state_idx]
        pop_real = np.real(pop_complex)
        # Handle both scalar and array cases
        if hasattr(pop_real, 'item'):
            pop_scalar = float(pop_real.item())
        else:
            pop_scalar = float(pop_real)
