"""
Final Working AWS FMO Demonstration - correctly handles AWS complex number format
"""
...
def extract_single_excitation_populations(density_matrix):
    """Extract populations - handles both local (numpy complex) and AWS ([real,imag] lists) formats."""
    
    # Get raw data
    if hasattr(density_matrix, 'value'):
        dm_raw = density_matrix.value
    else:
        dm_raw = density_matrix
    
    # Check if this is AWS format (nested lists with [real, imag])
    if isinstance(dm_raw, list) and isinstance(dm_raw[0], list) and isinstance(dm_raw[0][0], list):
        # AWS format: convert [real, imag] lists to complex numbers
        dm_shape = len(dm_raw)
        dm = np.zeros((dm_shape, dm_shape), dtype=complex)
        for i in range(dm_shape):
            for j in range(dm_shape):
                real_part, imag_part = dm_raw[i][j]
                dm[i, j] = complex(real_part, imag_part)
    else:
        # Local format: already numpy array or simple nested list
        dm = np.array(dm_raw)
    ...
    for state_idx in single_exc_states:
        # Extract diagonal element and get real part
        pop_complex = dm[state_idx, state_idx]
        pop_real = np.real(pop_complex)
        pop_scalar = float(pop_real)  # This should now work
        populations.append(pop_scalar)
...
if __name__ == "__main__":
    ...
    # Show noise-assisted transport success
    noiseless_aws = df_aws[df_aws['gamma'] == 0.0]['efficiency'].iloc[0]
    max_aws = df_aws['efficiency'].max()
    enhancement = (max_aws - noiseless_aws) / noiseless_aws * 100
    
    print(f"\n🎯 AWS Results Summary:")
    print(f"   Noiseless efficiency: {noiseless_aws:.4f}")
    print(f"   Maximum efficiency: {max_aws:.4f}")
    print(f"   Noise enhancement: {enhancement:.1f}%")
    
    if enhancement > 5:
        print(f"   ✅ SUCCESS: Noise-assisted transport confirmed on AWS!")
    else:
        print(f"   ⚠️  Enhancement below threshold")
