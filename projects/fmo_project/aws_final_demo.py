"""
Final AWS FMO Demonstration - properly handles all numpy scalar conversions
"""
...
def extract_single_excitation_populations(density_matrix):
    ...
    for state_idx in single_exc_states:
        # Extract the diagonal element and convert to Python float
        pop_complex = dm[state_idx, state_idx]
        pop_real = np.real(pop_complex)
        pop_scalar = float(pop_real.item())  # Use .item() to ensure scalar conversion
