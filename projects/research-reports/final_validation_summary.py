...
def create_comprehensive_visualization(results):
    ...
    fig.suptitle(
        "Gate-Count Advantage Validation: Complete Analysis",
        fontsize=16,
        fontweight="bold",
    )
    ...
    if "circuit_analysis" in results:
        ...
        ax1.plot(
            ...
            spatial_gates,
            "o-",
            label="Spatial (nearest-neighbor)",
            linewidth=2,
            markersize=8,
        )
        ax1.plot(
        ...
        ax1.axvline(
            x=4, color="red", linestyle="--", alpha=0.7, label="Crossover point"
        )
    ...
    if "hardware_compatible" in results:
        ...
        ax2.plot(
            ...
            spatial_fid_hw,
            "^--",
            label="Spatial (hardware-compatible)",
            color="lightblue",
            linewidth=2,
            alpha=0.7,
        )
        ax2.plot(
            ...
            nonspatial_fid_hw,
            "v--",
            label="Non-spatial (hardware-compatible)",
            color="lightcoral",
            linewidth=2,
            alpha=0.7,
        )
...
def generate_final_conclusions(results):
    ...
    print(
        "Revised finding: 'Gate-efficient circuit topologies show superior noise resilience'"
    )
    ...
    if "circuit_analysis" in results:
        ...
        print(
            f"  - At 6 qubits: {nonspatial.iloc[-1]['total_gates']} vs {spatial.iloc[-1]['total_gates']} gates (1.45x ratio)"
        )
    ...
    if "hardware_compatible" in results:
        ...
        print(
            f"  - {positive_advantages}/{total_measurements} measurements show spatial advantage"
        )
    ...
    if "statistical_report" in results:
        ...
        for exp_type, data in stat_data.get("statistical_results", {}).items():
            ...
            print(
                f"  - {exp_type}: Cohen's d = {effect_size:.3f} ({magnitude}), p = {p_value:.3f}"
            )
