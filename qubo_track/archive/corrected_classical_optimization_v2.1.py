...
def compute_exact_max_cut(graph: nx.Graph, quick_mode: bool = False) -> int:
    ...
    # Quick mode: use Tabu approximation for larger graphs (FIXED: moved to top)
    if quick_mode and n > 10:
        print(f"    Quick mode: Using Tabu approximation for {n}-node graph...")
...
class CorrectedClassicalComparison:
    ...
    def run_statistical_comparison(self):
        ...
        for graph_name, graph in self.graphs.items():
            ...
            # FIXED: NaN-aware Cohen's d reporting with interpretation
            if np.isnan(cohens_d) or pooled_std == 0:
