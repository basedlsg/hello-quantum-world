...
def create_padded_spatial_circuit():
    ...
    if padding_needed < 0:
        raise ValueError("GATE_BUDGET is smaller than the base number of spatial CNOTs.")
...
def create_sampled_nonspatial_circuit(seed: int):
    ...
    for q1, q2 in chosen_edges: c.cnot(q1, q2)
    return c
...
def create_random_er_circuit(seed: int):
    ...
    c = Circuit().h(range(N_QUBITS))
    for u, v in G.edges(): c.cnot(u, v)
    return c

def count_cnots(c: Circuit) -> int:
    return sum(1 for i in c.instructions if isinstance(i, instruction.Instruction) and i.operator.name == 'CNot')

# --- Main Logic ---
def run_parity_experiment(circuit_type: str, device, instances=1):
    results = []
    print(f"\n--- Running Parity Check: {circuit_type} ({instances} instances) ---", flush=True)
    ...
    for i in range(instances):
        ...
        print(f"  Instance {i+1}/{instances} (Seed: {seed}): Building circuits...", flush=True)
        ...
        assert cnot_count == GATE_BUDGET, f"[{circuit_type}] Base circuit CNOT count is wrong! Expected {GATE_BUDGET}, got {cnot_count}"
        noisy_circuit = apply_noise_to_circuit(base_circuit)
        noisy_cnot_count = count_cnots(noisy_circuit)
        assert noisy_cnot_count == GATE_BUDGET, f"[{circuit_type}] Noisy circuit CNOT count is wrong! Expected {GATE_BUDGET}, got {noisy_cnot_count}"
        # =======================================================
        ...
        results.append({'circuit_type': circuit_type, 'instance': i, 'fidelity': fidelity, 'min_eigenvalue': min_eig, 'cnot_count': cnot_count, 'seed': seed})
        if elapsed > 400:
...
def fidelity_robust(rho, sigma):
    ...
    return max(0.0, min(1.0, np.real(overlap.item() if isinstance(overlap, np.ndarray) else overlap)))

def main():
    parser = argparse.ArgumentParser(description="Run Gate-Count Parity Check Experiment.")
    parser.add_argument("--device_arn", type=str, default="arn:aws:braket:::device/quantum-simulator/amazon/dm1", help="AWS Braket device ARN. Use 'local' for local simulation.")
    args = parser.parse_args()

    device = AwsDevice(args.device_arn) if args.device_arn != "local" else LocalSimulator("braket_dm")
    ...
    nonspatial_results = run_parity_experiment('nonspatial_sampled', device, instances=RANDOM_INSTANCES)
    random_results = run_parity_experiment('random_er', device, instances=RANDOM_INSTANCES)

    all_results = pd.DataFrame(spatial_results + nonspatial_results + random_results)
    
    device_name = "local" if args.device_arn == "local" else device.name.replace('/', '_')
